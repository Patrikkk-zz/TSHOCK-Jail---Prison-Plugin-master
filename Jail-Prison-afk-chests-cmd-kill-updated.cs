...
namespace JailPrison
{
    ...
    public class Jail : TerrariaPlugin
    {
                        ...
                        if (player != null && player.TSPlayer != null)
                        {
                            ...
                            if (player.prisonMode && JPConfig.prisonmode)
                            {
                                if (!currentregionlist.Contains("prison"))
                                {
                                    ...
                                    {
                                        if (player.TSPlayer.Teleport((int)warp.WarpPos.X*16, (int)warp.WarpPos.Y*16 + 3))
                                        {
        ...
        private static void sendback(CommandArgs args)
        {
            ...
            {
                if (args.Player.Teleport(Players[GetPlayerIndex(args.Player.Index)].backtileX*16, Players[GetPlayerIndex(args.Player.Index)].backtileY*16 + 3))
                {
